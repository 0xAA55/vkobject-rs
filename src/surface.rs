...
impl VulkanSurface {
	...
	pub fn new_from(vkcore: Arc<VkCore>, surface: VkSurfaceKHR, format: VkSurfaceFormatKHR) -> Self {
		Self {
			vkcore,
			surface,
			format,
	...
	/// Create a new `VulkanSurface` from the generic type `surface_ci` by calling the given function pointer of `vkCreateSurfaceXxxx()`
	#[allow(dead_code)]
	fn new_from_ci<T>(function_name: &'static str, vkcore: Arc<VkCore>, device: &VulkanDevice, vk_create_surface: fn(VkInstance, &T, *const VkAllocationCallbacks, *mut VkSurfaceKHR) -> VkResult, surface_ci: &T) -> Result<Self, VulkanError> {
		...
		let surface = ResourceGuard::new(surface, |&s|{let vkcore = vkcore.clone(); vkcore.vkDestroySurfaceKHR(vkcore.get_instance(), s, null()).unwrap()});
		...
		let surface = surface.release();
		Ok(Self::new_from(vkcore, surface, selected_format))
	}
	/// Create a `VulkanSurface` by using the GLFW API
	#[cfg(any(feature = "glfw", test))]
	pub fn new(vkcore: Arc<VkCore>, device: &VulkanDevice, window: &glfw::PWindow) -> Result<Self, VulkanError> {
		Self::new_from_ci("vkCreateWindowSurfaceGLFW", vkcore, device, vkCreateWindowSurfaceGLFW, window)
	}
	/// Create a `VulkanSurface` by using the Win32 API
	#[cfg(feature = "win32_khr")]
	pub fn new(vkcore: Arc<VkCore>, device: &VulkanDevice, hwnd: HWND, hinstance: HINSTANCE) -> Result<Self, VulkanError> {
	...
	/// Create a `VulkanSurface` for Android
	#[cfg(feature = "android_khr")]
	pub fn new(vkcore: Arc<VkCore>, device: &VulkanDevice, window: *const ANativeWindow) -> Result<Self, VulkanError> {
	...
	/// Create a `VulkanSurface` for IOS
	#[cfg(feature = "ios_mvk")]
	pub fn new(vkcore: Arc<VkCore>, device: &VulkanDevice, view: *const c_void) -> Result<Self, VulkanError> {
	...
	/// Create a `VulkanSurface` for MacOS
	...
	/// Create a `VulkanSurface` for Metal
	#[cfg(feature = "metal_ext")]
	pub fn new(vkcore: Arc<VkCore>, device: &VulkanDevice, metal_layer: *const CAMetalLayer) -> Result<Self, VulkanError> {
	...
	/// Create a `VulkanSurface` for Wayland
	#[cfg(feature = "wayland_khr")]
	pub fn new(vkcore: Arc<VkCore>, device: &VulkanDevice, display: *const c_void, surface: *const c_void) -> Result<Self, VulkanError> {
	...
	/// Create a `VulkanSurface` for XCB
	#[cfg(feature = "xcb_khr")]
	pub fn new(vkcore: Arc<VkCore>, device: &VulkanDevice, connection: *const c_void, window: xcb_window_t) -> Result<Self, VulkanError> {
...
impl Drop for VulkanSurface {
	fn drop(&mut self) {
		self.vkcore.vkDestroySurfaceKHR(self.vkcore.get_instance(), self.surface, null()).unwrap();
	}
}
