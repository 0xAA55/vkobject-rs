...
impl MaterialComponent {
	...
	pub fn from_obj(device: Arc<VulkanDevice>, cmdbuf: VkCommandBuffer, obj_comp: &ObjMaterialComponent) -> Result<Self, VulkanError> {
		match obj_comp {
			ObjMaterialComponent::Texture(path) => Ok(MaterialComponent::Texture(Arc::new(VulkanTexture::new_from_path(device, cmdbuf, path, true, Some(VkFilter::VK_FILTER_LINEAR), VkImageUsageFlagBits::VK_IMAGE_USAGE_SAMPLED_BIT as VkImageUsageFlags)?))),
...
impl Material for MaterialLegacy {
	...
	fn get_ao(&self) ->				Option<&MaterialComponent> {Some(&self.ambient)}
...
impl Material for MaterialPbr {
	...
	fn get_ambient(&self) ->		Option<&MaterialComponent> {Some(&self.ao)}
